...
class Shaker(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        super().__init__()
        self.game = game
        
        # Load Pot Body
        self.image = pygame.image.load(os.path.join("assets", "images", "Shaker.png")).convert_alpha()
        self.image = pygame.transform.smoothscale(self.image, (300, 300))
        self.is_shaking = False 
        self.shake_start_time = 0
        self.shake_duration = 3000 # 3 seconds
        
        # Load Pot Lid
        try:
            self.lid_image = pygame.image.load(os.path.join("assets", "images", "Shaker_lid.png")).convert_alpha()
            self.lid_image = pygame.transform.smoothscale(self.lid_image, (300, 300))
        except:
            self.lid_image = None # Fallback if lid missing
    ...
    def update(self):
        ...
        if self.is_shaking:
            ...
            if now - self.shake_start_time > self.shake_duration:
                self.is_shaking = False # Open the lid!
